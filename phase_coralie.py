...
df = df.loc[(df['CORALIE'] == 1)].reset_index(drop=True)
...
coralie = pd.read_csv('coralie_noise_data.csv', sep=',', names=['x','y']) #read CORALIE noise data (minimum signal that gives 3sigma detection)
...
CORALIE_err = (coralie_func(Vmag)/3.)
print('CORALIE_err=', CORALIE_err)
...
while k<len(df):
        ...
        period = 10.**log_period# + (1./(24.*60.*60.))
        ...
        while j<jlen:
                ...
                while (abs(pgram_period-period)/period > 0.05) | ((abs(pgram_K-K)/K) > 0.05) | (abs(pgram_b) > 0.05):
                        ...
                        if good_observing != 0:
                                ...
                                phase = time
                                amplitude = K*np.sin(phase*2.*np.pi/period) + np.random.normal(0,CORALIE_err)
                                PHASE.append(phase)
                        ...
                        if ((time - int_time)>=0.5*period) & (len(PHASE)>=6):
                                    ...
                                    if (abs(pgram_period - period)/period < 0.05):
                                        ...
                                        try:
                                            popt_test = curve_fit(func_test, ( PHASE/pgram_period - divmod(PHASE/pgram_period, 1)[0] ), AMPLITUDE, sigma=np.ones(len(PHASE))*CORALIE_err, bounds=([-np.inf,-0.5], [np.inf, 0.5]))[0]
                                            pgram_K = popt_test[0]
                                            pgram_b = popt_test[1]
                                        except:
                                            pgram_K = 0
                        ...
                        if (day_time/period > 10.):# & (day_time > 100.):
                            print('limit reached')
                            pgram_period = period
                            pgram_K = K
                            pgram_b = 0.
...
df.to_csv('phase_no_phase_pgram_short_coralie_out_post_ref_40.csv', index=False)
